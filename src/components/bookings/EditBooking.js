    ...
    async function updateBooking(e) {
        ...
        if (Object.keys(validationErrors).length === 0) {
            ...
            try {
                ...
                headers.append('Accept', 'application/json');
                headers.append('authorization', 'Bearer ' + TOKEN);
                let res = await fetch(apiBaseUrl + "/booking/"+infoId, {
                    method: "PUT",
    ...
    useEffect(() => {  
        const getInformation = async (infoId) => {
            try {
                ...
                headers.append('Accept', 'application/json');
                headers.append('authorization', 'Bearer ' + TOKEN);
                let res = await fetch(apiBaseUrl+"/booking/info/" + infoId, {
                    method: "GET",
                    headers: headers
                });
                let resJson = await res.json();
                if (resJson.status) {
                    const sTime = moment(new Date(resJson.result.start_time)).format("YYYY-MM-DD HH:mm");
                    resJson.result['start_time'] = sTime;
                    const eTime = moment(new Date(resJson.result.end_time)).format("YYYY-MM-DD HH:mm");
                    resJson.result['end_time'] = eTime;
        ...
        <>
            <Layout>
                <div className="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                    <h1 className="h4">Edit Booked Room </h1>
                                ...
                                <div className="col-sm-12">
                                    ...
                                    <select className="form-control" name="room_id" value={inputFormData.room_id} onChange={handleInputChange} >
                                        ...
                                        {rooms?.map((room, index) => (
                                            <option value={room._id} key={index}>{room.room_name}</option>
                                        ))}
                                    </select>
                                </div>
                            ...
                            <div className="col-md-6" key={index}>
                                <div className="row g-0 border rounded overflow-hidden flex-md-row mb-4 shadow-sm h-md-250 position-relative">
                                    <div className="col p-4 d-flex flex-column position-static">
                                        <strong className="d-inline-block mb-2 text-success">{alreadyBooking.purpose}</strong>
                                        {/* <h3 className="mb-0">{alreadyBooking.purpose}</h3> */}
